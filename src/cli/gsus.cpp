...
int main(int argc, char** argv) {
    ...
    int rc = 0;
    if (cmd == "echo") {
        if (argc < 3) { fprintf(stderr, "echo requires a string\n"); rc = 1; }
        else rc = call_echo(bus, argv[2]);
    } else if (cmd == "version") {
        rc = call_version(bus);
    } else if (cmd == "list") {
        rc = call_list(bus);
    } else if (cmd == "add") {
        if (argc < 3) { fprintf(stderr, "add requires an item name\n"); rc = 1; }
        else rc = call_add(bus, argv[2]);
    } else {
